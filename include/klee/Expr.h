...
public:
  Expr() : refCount(0), shadowflag(false),PointerFlag(false) { Expr::count++; }
...
//bupt use
//shadowExpr inherited from BinaryExpr. left for old version and right for new version.
class ShadowExpr: public BinaryExpr{
public:
    static const Kind kind = Shadow;
...
public:
    ...
    Kind getKind() const { return Shadow; }
    virtual ref<Expr> rebuild(ref<Expr> kids[]) const{
        return create(kids[0],kids[1]);
    }
    static bool classof(const Expr *E){
        return E->getKind() == Expr::Shadow;
    }
    static bool classof(const ShadowExpr *){
        return true;
    }
};
