...
namespace {
  ...
  //bupt use
  cl::list<std::string>
      ConcernFunction("concern-function",cl::ZeroOrMore);
}
...
extern std::map<std::string, int> concernFunction;
extern std::map<std::string, int> dataDivInCF;
//extern std::map<std::string, std::map<std::string,int> > dataDivInCF;
std::map<std::string,std::map<const unsigned int,int> > ctlrecord;
extern std::vector<char*> buf_for_argv;
extern bool is_print;
extern std::string printfunc;

int data_num=0;
int ctrl_num=0;
int data_ctrlAffected=0;
/***/
...
void KleeHandler::processTestCase(const ExecutionState &state,
  ...
  if (!NoOutput) {
	...
	//bupt use
     ...
     if(BuptShadow && state.needTestCase)
     {
        ...
        if(errorMessage)
        {
            ...
            *f << state.divmsg;
            ...
            if(is_print)
            {
                if(! dataDivInCF[printfunc])
                    dataDivInCF[printfunc]=1;
                else
                    dataDivInCF[printfunc]++;
            }
            else
            {
                std::string funcname=state.prevPC->inst->getParent()->getParent()->getName().str();
                if(!dataDivInCF[funcname])
                    dataDivInCF[funcname]=1;
                else
                    dataDivInCF[funcname]++;
            }
            data_num++;
        }
        //  dive_info="data flow divergence";
            ...
            std::string file=bufpc->info->file.c_str();
            std::map<std::string, std::map<const unsigned int,int> >::iterator itc=ctlrecord.find(file);
            if(itc==ctlrecord.end())
                ctlrecord[file][bufpc->info->line]=1;
            else{
                std::map<const unsigned int,int>::iterator sit=itc->second.find(bufpc->info->line);
                if(sit==itc->second.end()){
                    ctlrecord[file][bufpc->info->line]=1;
                } else {
                    ctlrecord[file][bufpc->info->line]++;
                }
            }
            ctrl_num++;
            if(bufpc->info->file!="")
        ...
        //  dive_info="control flow divergence";
        //klee_message("KTEST FILE %s: divergence type: %s\n", getTestFilename("ktest", id).c_str(),dive_info.c_str());
        //klee_message("KTEST FILE %s: divergence type: %s\n", dive_info,dive_info);
        if(state.ctlordata && state.ctlAffected)
        {
            klee_message("This data divergence is affected by control divergence\n");
            data_ctrlAffected++;
        }
        else if(state.ctlordata)
            klee_message("This data divergence is not affected by control divergence\n");
      }
      else
	{
		if (!kTest_toFile(&b, getOutputFilename(getTestFilename("ktest", id)).c_str()))
...
static const char *modelledExternals[] = {
  ...
  "klee_stack_trace",
//bupt use
  "klee_shadow_enabled", // enable symbolic input files
  "klee_get_concrete_value", // always get concrete value for symbolics
...
static llvm::Module* MakeArgsSymbolic(llvm::Module *mainModule)
{
	...
	if (userMain && userMain->arg_size() >= 2) {
		...
		if (makeSymbolic) {
            //std::cerr<<"in main mark args symbolic\n";
			std::vector<llvm::Value*> args;
...
//bupt use
  ...
  for (unsigned i=0; i<InputArgv.size()+1; i++) {
	...
	if(i!=0)
	{
		char *buf_arg=new char[size];
		std::copy(pArg, pArg+size, buf_arg);
		buf_for_argv.push_back(buf_arg);
	}
  }
  ...
  klee_message("input args: %s",msg.str().c_str());
  infoFile << "PID: " << getpid() << "\n";
...
//bupt use
  ...
  stats << "KLEE: data flow divergences discovered in concerned functions:\n";
    for (std::map<std::string,int>::iterator
           it_func = concernFunction.begin(), ie_func = concernFunction.end();
         it_func != ie_func; ++it_func) {
        stats << "function "<<it_func->first<<" in level "<<it_func->second
              << " founded "<<dataDivInCF[it_func->first]<<" data divergence\n";
    }
  stats << "KLEE: control flow divergences discovered:\n";
    for (std::map<std::string,std::map< const unsigned int,int> >::iterator
           it_func = ctlrecord.begin(), ie_func =ctlrecord.end();
         it_func != ie_func; ++it_func) {
        for (std::map<const unsigned int,int>::iterator
               it_info = it_func->second.begin(), ie_info =it_func->second.end();
               it_info != ie_info; ++it_info) {
            stats << it_info->second <<" control flow divergences are found at File "<<it_func->first
                <<" Line: "<<it_info->first<<"\n";
        }
    }
    stats << "Total data divergences:"<<data_num << "\n";
    stats << data_ctrlAffected << "   of data divergences are affected by control divergences\n";
    stats << "Total control divergences:"<<ctrl_num << "\n";
