...
const char *__concretize_buffer(const char *s, unsigned size);
...
static exe_disk_file_t *__get_sym_file_internal(const char *pathname, int flags, bool cp_treat_symbolic) {
  ...
  if (!pathname || !pathname[0])
    return NULL;
  UseConcretePath = (klee_shadow_enabled() == 1);
...
ssize_t read(int fd, void *buf, size_t count) {
  ...
  if (!f->dfile) {
    ...
    void* incoming = buf;
    if ((f->flags & eSocket)  && klee_shadow_enabled()) {
