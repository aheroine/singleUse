...
private:
  ...
  /// Depended on Context::get().getPointerWidth(), decided whether the architecture
  /// is x86_64 (isX86_64==true) or x86_32(isX86_64==false)
  bool isX86_64;
  ...
  StatePair fork(ExecutionState &current, ref<Expr> condition, bool isInternal, bool isFWForking);
...
//bupt use
  void getCallers(Function *fn,std::set<Function*> &callChain,int level);
  void chainCreate(BasicBlock *root, BasicBlock* end, CFGPath &currentPath,int mode);
  void chainCreate(ExecutionState::condsWithMG &coveredMap, BasicBlock *root, BasicBlock* end, CFGPath &currentPath, CFGPath &coveredBBs, CFGPath &mgBBs);
  void findCoveredBB(BasicBlock *root,CFGPath &currentPath);
  void findExit(BasicBlock *root, std::vector<BasicBlock*> &exitSet,BasicBlock *ends);
  bool computeMerge(ExecutionState &state, llvm::BasicBlock *left, llvm::BasicBlock *right);
  BasicBlock* uniqueMergePoint(BasicBlock *mgbb);
  bool handlePointer(ref<Expr> value, std::map <const MemoryObject*, std::vector<  ref<Expr> > > &record,std::set<const MemoryObject*> &allRelatedMems, ExecutionState &state, bool tc, int stage);
  unsigned getBlockHeight(BasicBlock *start, std::map<BasicBlock*, unsigned> &BlockHeight, std::set<BasicBlock*> &visitedBlock);


  void createTestCaseButNoTerminate(ExecutionState &state);
void detectDataDiv(ExecutionState &state, ref<Expr> value, unsigned size=0);

void shadowReadOrWrite(ExecutionState &state,
        bool isWrite,
        Expr::Width type,
        KInstruction *target,
        ref<Expr> old_addr,
...
void normalReadOrWrite(ExecutionState &state,
        bool isWrite,
        ref<Expr> offset,
        Expr::Width type,
        ref<Expr> address,
