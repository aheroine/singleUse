...
void DFSSearcher::update(ExecutionState *current,
		...
		for (std::set<ExecutionState*>::const_iterator it = addedStates.begin(),
		 ie = addedStates.end(); it != ie; ++it) {
			...
			if(es->isSPEN())
				...
				std::vector<ExecutionState*>::iterator sit=find(states.begin(),states.end(),es);
				if(sit==states.end())
					states.push_back(es);
			}
		}
  ...
  for (std::set<ExecutionState*>::const_iterator it = removedStates.begin(),
    ...
    if (es == states.back()) {
      ...
      //XXX: check all the states added to removedStates!
    }
  }
  /*
    errs()<<"searcher states after update:\n";
    for(std::vector<ExecutionState*>::iterator it=states.begin(),ie=states.end();it!=ie;it++){
          errs()<<*it<<"\n";
    }
    */
