...
//bupt use
ref<Expr> Expr::findShadowExpr(unsigned numKid){
    if(this->getKind()==Expr::Constant)
    {
        return this;
    }
    if(this->getKind()==Expr::Shadow )
    {
        ref<Expr> shadExpr=this->getKid(numKid);
        //Final level of shadowExpr
        if(shadExpr->isContainShadow())
        {
            return shadExpr->findShadowExpr(numKid);
        }
        else
        {
            if(this->isCtrlAffected())
                shadExpr->setOriginFlag();
            return shadExpr;
        }
    }
    ref<Expr> kids[8];
   // std::cerr<<"this contains shadow? "<<this->isContainShadow()<<std::endl;
    if(this->isContainShadow()){
        for(unsigned i=0;i<this->getNumKids();i++){
           if(this->getKid(i)->isContainShadow())
           {
                kids[i]=(this->getKid(i))->findShadowExpr(numKid);
           } else{
                kids[i]=this->getKid(i);
           }
        }
        if(ReadExpr *re=dyn_cast<ReadExpr>(this)){
            const UpdateList &ul=re->updates;
            const Array *new_array=re->updates.root;
            UpdateList new_ul = UpdateList(new_array,0);
            ref<Expr> idx,val;
            for(const UpdateNode *un=ul.head;un;un=un->next){
                idx=un->index->findShadowExpr(numKid);
                val=un->value->findShadowExpr(numKid);
                new_ul.extend(idx,val);
            }
            ref<Expr> res=ReadExpr::create(new_ul,kids[0]);
            if(this->isCtrlAffected())
                res->setOriginFlag();
            return res;
        }
        ref<Expr> result=this->rebuild(kids);
        if(this->isCtrlAffected())
            result->setOriginFlag();
        return result;
    } else
        return this;
}
...
static ref<Expr> SubExpr_createPartialR(const ref<ConstantExpr> &cl, Expr *r) {
  ...
  ref<Expr> result;
  if (type==Expr::Bool) {
    result=XorExpr_createPartialR(cl, r);
    //return XorExpr_createPartialR(cl, r);
  } else {
    Expr::Kind rk = r->getKind();
    if (rk==Expr::Add && isa<ConstantExpr>(r->getKid(0))) { // A - (B+c) == (A-B) - c
      result=SubExpr::create(SubExpr::create(cl, r->getKid(0)),
                             r->getKid(1));
      //return SubExpr::create(SubExpr::create(cl, r->getKid(0)),
      //                       r->getKid(1));
    } else if (rk==Expr::Sub && isa<ConstantExpr>(r->getKid(0))) { // A - (B-c) == (A-B) + c
      result=AddExpr::create(SubExpr::create(cl, r->getKid(0)),
                             r->getKid(1));
      //return AddExpr::create(SubExpr::create(cl, r->getKid(0)),
      //                       r->getKid(1));
    } else {
      result=SubExpr::alloc(cl, r);
      //return SubExpr::alloc(cl, r);
    }
  }
  if(cl->isPointer() && r->isPointer())
...
static ref<Expr> SubExpr_create(Expr *l, Expr *r) {
  ...
  ref<Expr> result;
  if (type == Expr::Bool) {
    result=XorExpr_create(l, r);
    //return XorExpr_create(l, r);
  } else if (*l==*r) {
    result=ConstantExpr::alloc(0, type);
    //return ConstantExpr::alloc(0, type);
  } else {
    Expr::Kind lk = l->getKind(), rk = r->getKind();
    if (lk==Expr::Add && isa<ConstantExpr>(l->getKid(0))) { // (k+a)-b = k+(a-b)
      result=AddExpr::create(l->getKid(0),
                             SubExpr::create(l->getKid(1), r));
    } else if (lk==Expr::Sub && isa<ConstantExpr>(l->getKid(0))) { // (k-a)-b = k-(a+b)
      result=SubExpr::create(l->getKid(0),
                             AddExpr::create(l->getKid(1), r));
    } else if (rk==Expr::Add && isa<ConstantExpr>(r->getKid(0))) { // a - (k+b) = (a-c) - k
      result=SubExpr::create(SubExpr::create(l, r->getKid(1)),
                             r->getKid(0));
    } else if (rk==Expr::Sub && isa<ConstantExpr>(r->getKid(0))) { // a - (k-b) = (a+b) - k
      result=SubExpr::create(AddExpr::create(l, r->getKid(1)),
                             r->getKid(0));
    } else {
      result=SubExpr::alloc(l, r);
    }
    /*
    if (lk==Expr::Add && isa<ConstantExpr>(l->getKid(0))) { // (k+a)-b = k+(a-b)
      return AddExpr::create(l->getKid(0),
...
static ref<Expr> MulExpr_create(Expr *l, Expr *r) {
  ...
  */
  if (type == Expr::Bool) {
    result=AndExpr::alloc(l, r);
  } else {
    result=MulExpr::alloc(l, r);
  }
  if(ConstantExpr *ce=dyn_cast<ConstantExpr>(r)){
      if(ce->isZero())
        result->setIsNotPointer();
  } else if (ConstantExpr *ce=dyn_cast<ConstantExpr>(l)){
      if(ce->isZero())
        result->setIsNotPointer();
  }
  return result;
...
static ref<Expr> AndExpr_createPartial(Expr *l, const ref<ConstantExpr> &cr) {
  if (cr->isAllOnes()) {
    ...
    ref<Expr> result= AndExpr::alloc(l, cr);
    // if MSB(cr)==0
    ref<ConstantExpr> ccr=cr->Extract(cr->getWidth(),cr->getWidth());
    if(ccr->isZero())
        result->setIsNotPointer();
    return result;
...
static ref<Expr> AndExpr_create(Expr *l, Expr *r) {
  //return AndExpr::alloc(l, r);
    ref<Expr> result= AndExpr::alloc(l, r);
    if(ConstantExpr *ce=dyn_cast<ConstantExpr>(r)){
        // if MSB(r)==0
        ref<ConstantExpr> cr=ce->Extract(ce->getWidth(),ce->getWidth());
        if(cr->isZero())
            result->setIsNotPointer();
    } else if (ConstantExpr *ce=dyn_cast<ConstantExpr>(l)){
       // if MSB(l)==0
        ref<ConstantExpr> cl=ce->Extract(ce->getWidth(),ce->getWidth());
        if(cl->isZero())
            result->setIsNotPointer();
    }
    return result;
...
static ref<Expr> UDivExpr_create(const ref<Expr> &l, const ref<Expr> &r) {
  if (l->getWidth() == Expr::Bool) { // r must be 1
    ...
    ref<Expr> result= UDivExpr::alloc(l,r);
    if(l->isPointer())
      result->setIsPointer();
    return result;
...
static ref<Expr> SDivExpr_create(const ref<Expr> &l, const ref<Expr> &r) {
  if (l->getWidth() == Expr::Bool) { // r must be 1
    ...
    ref<Expr> result= SDivExpr::alloc(l,r);
    if(l->isPointer())
      result->setIsPointer();
    return result;
...
static ref<Expr> ShlExpr_create(const ref<Expr> &l, const ref<Expr> &r) {
  ...
  */
  ref<Expr> result;
  if (l->getWidth() == Expr::Bool) { // l & !r
      result= AndExpr::create(l, Expr::createIsZero(r));
  } else{
      result= ShlExpr::alloc(l, r);
  }
  result->setIsNotPointer();
  return result;
}
...
//bupt use
//ShadowExpr create function
ref<Expr> ShadowExpr::create(const ref<Expr> &l, const ref<Expr> &r){
   //return ShadowExpr::ShadowExpr_create(l,r);
   return ShadowExpr::alloc(l,r);
}
