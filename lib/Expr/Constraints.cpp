...
bool ConstraintManager::rewriteConstraintsInMerge(ExprVisitor &visitor, bool &succ) {
  ConstraintManager::constraints_ty old;
  bool changed = false;
  ...
  for (ConstraintManager::constraints_ty::iterator
    ...
    if (e!=ce) {
      succ=addConstraintInternalInMerge(e); // enable further reductions
      changed = true;
    } else {
      constraints.push_back(ce);
    }
...
bool ConstraintManager::addConstraintInternalInMerge(ref<Expr> e) {
  ...
  case Expr::And: {
    ...
    no_conflict=addConstraintInternalInMerge(be->left);
    no_conflict=addConstraintInternalInMerge(be->right);
    break;
  }
  ...
  case Expr::Eq: {
    if (RewriteEqualities) {
      ...
      if (isa<ConstantExpr>(be->left)) {
        ...
        rewriteConstraintsInMerge(visitor,no_conflict);
      }
    }
  ...
  switch (e->getKind()) {
      case Expr::Constant:{
                              if(debug_internal){
                                    ...
                                    ExprPPrinter::printConstraints(info, *this);
                                    llvm::errs()<<"current added constraint: \n"<<e<<"\n";
...
void ConstraintManager::addConstraint(ref<Expr> e) {
  if(debug_internal){
        ...
        ExprPPrinter::printConstraints(info, *this);
        llvm::errs()<<"------------------------\n";
        llvm::errs()<<"current simplifying expr: \n"<<e<<"\n";
        llvm::errs()<<"current constraints:\n"<<info.str()<<"\n";
  }
  e = simplifyExpr(e);
  if(debug_internal){
      llvm::errs()<<"after simplified: "<<e<<"\n";
      llvm::errs()<<"========================\n";
  }
  addConstraintInternal(e);
}

bool ConstraintManager::addConstraintInMerge(ref<Expr> e) {
  e = simplifyExpr(e);
  return addConstraintInternalInMerge(e);
}
